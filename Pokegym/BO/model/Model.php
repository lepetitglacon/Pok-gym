...
class Model {
    ...
    public function getAllUserWithoutPassword(){
        $sql = 'SELECT `id`, `name`, `surname`, `email`, `newsletter`, `bo` FROM `USER`';
        $req = $this->pdo->prepare($sql);
        $req->execute();
        return $req->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public function getUserAdministrator(){
        $sql = 'SELECT * FROM USER WHERE `bo` = 2';
        $req = $this->pdo->prepare($sql);
        $req->execute();
        return $req->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    */
    ...
    public function updateUserBo($id,$bo){
        $sql = 'UPDATE `USER` SET `bo`= :bo WHERE `id` = :id';
        $req = $this->pdo->prepare($sql);
        $req->bindparam('id', $id);
        $req->bindparam('bo', $bo);
        return $req->execute();
    ...
    public function getRubrique(){
        $sql = 'SELECT id,title, dropdown FROM `RUBRIQUE`  GROUP BY `title` ORDER BY id ASC';
        $req = $this->pdo->prepare($sql);
        $reponse = $req->execute();
        return $req->fetchAll(PDO::FETCH_NAMED);
    }

    public function insertRubrique($title,$dropdown){
        $sql = 'INSERT INTO `RUBRIQUE`(`title`, `dropdown`) VALUES (:title,:dropdown)';
        $req = $this->pdo->prepare($sql);
        $req->bindparam('title',$title);
        $req->bindparam('dropdown',$dropdown);
        return $req->execute();
    }
